...
const PARAGRAPH_CONTENT = `<w:p><w:r><w:t>%s</w:t></w:r></w:p>`
...
func (d *Docx) AddParagraph(content []string) {
	reg := regexp.MustCompile(`<w:p.*<\/w:p>`)

	var build strings.Builder
	for _, item := range content {
		itemstr, _ := encode(item)
		contentstr := fmt.Sprintf(PARAGRAPH_CONTENT, itemstr)
		build.WriteString(contentstr)
	}

	d.content = reg.ReplaceAllString(d.content, build.String())
}
...
func TxtToDocx(txtPath string, docxPath string) error {
	...
	for scanner.Scan() {
		...
		textContent = append(textContent, lineText)
	}

	// Read from docx file
	r, err := ReadDocxFile("./template.docx")
	defer r.Close()
	// Or read from memory
	...
	docxedit := r.Editable()
	docxedit.AddParagraph(textContent)
	docxedit.WriteToFile(docxPath)

	return nil
}

func StringToDocx(content string) (string, error) {
